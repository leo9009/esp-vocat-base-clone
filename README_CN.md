...
VoCat 喵伴旋转底座是为[**VoCat 喵伴开发套件**](https://docs.espressif.com/projects/esp-dev-kits/zh_CN/latest/esp32s3/vocat/index.html)量身打造的智能旋转底座，专用于玩具、智能音箱、智能中控等需要大模型语音交互能力的产品。设备采用 ESP32-C61-WROOM-1 模组，支持 USB Type-C 供电，并可通过磁吸接口为 VoCat 本体供电。

该项目实现了**高精度步进电机控制、磁吸滑动开关事件检测、 CSI 感知功能**、稳定的 **UART 通信**等核心功能，同时能够根据 VoCat 的 **声源识别（Sound Source Localization）** 结果自动调整方向，实现面向声源的智能旋转。通过动作控制与声源感知的结合，提供更**自然、更具沉浸感**的人机交互体验。
...
VoCat 底座采用乐鑫 **ESP32-C61-WROOM-1(N8R2)** 模组，通过自定义的 UART 协议，与 VoCat 本体进行串口通信。机械结构方面，底座配备 24BYJ48 步进电机，可实现声源识别和多种预定义旋转动作，增强交互体验。
此外，VoCat 底座配有一个磁吸滑动开关，可通过磁吸滑动开关，解锁更多交互体验。
### **声源识别**
VoCat 支持声源识别功能，能够实时检测环境中的声音方向和位置。系统通过麦克风阵列采集声音信号，结合声强、相位等信息进行分析，判断声源所在的方位角。配合基于步进电机的超静音旋转底座，可以实现面向声源的智能交互。  
...
- **鼓点律动**
鼓点律动动作使 VoCat 能够根据外界音乐的**鼓点节奏**左右摆头，营造与音乐同步的**互动效果**。
...
![蹭手.gif](https://image.lceda.cn/oshwhub/pullImage/ca70534cecc342eea3f5e9ee2383a7a7.gif)
  ...
  **注意事项：**
  ...
  - **自动归位校准：** 
  由于 VoCat 旋转底座由步进电机驱动，因此没有角度反馈，故而底座在启动时将进行零位校准，校准流程如下：
   1. 上电后，电机会自动向左旋转寻找限位开关
   2. 触碰到限位开关后，电机会向右旋转 95° 到中心位置
   3. 校准完成后，电机断电
 
### **磁吸滑动开关交互控制**
VoCat 底座通过**磁吸式**滑动开关实现多种交互控制。滑块的不同位置会改变地磁传感器周围的磁场强度，底座通过实时监测这些磁场变化来识别滑块的动作。当检测到位置变化时，底座会将相应事件通过串口上报给 VoCat，从而实现丰富、直观的交互体验。
...
支持 9 种磁吸滑动开关事件检测：
   ...
   - **配对模式** (PAIRING)：两个 VoCat 面对面靠近，用于配对联网 
...
磁吸滑动开关控制功能支持**多种传感器**：
  ...
  **注意：**
  ...
  2. 在首次使用该功能前，为适配磁场环境，须完成磁吸滑动开关的**全自动校准**，以记录其处于 **上方 / 下方 / 移除** 三种状态时的磁场强度基准值：
     ...
     - 在使用过程中如需重新校准，可通过 VoCat 发送串口命令或长按底座 Boot 键进行触发
...
### **串口通信**
VoCat 旋转底座可通过 **UART 协议**与 VoCat 进行数据通信。
...
[**详细的命令数据帧示例**](software/vocat_rotating_base/README_CN.md)
