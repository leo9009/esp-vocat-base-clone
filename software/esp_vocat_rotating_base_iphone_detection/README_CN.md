# ESP-VoCat-Base: 喵伴旋转底座（iPhone 检测）

**中文** | [English](README.md)

## 项目简介

> **本例程专注于 iPhone 检测场景**：在综合例程（`esp_vocat_rotating_base`）的基础上，针对 iPhone 磁吸配件加载了 iPhone 专用磁场检测配置（`magnetic_slide_switch/profiles/iphone`），并注册了磁开关事件回调，将检测到的事件通过 UART 主动上报给 ESP-VoCat 主机。
...
└── slide_switch_event_detect_task // 磁吸滑动开关事件检测任务（iPhone 检测事件 → UART 上报）
```
