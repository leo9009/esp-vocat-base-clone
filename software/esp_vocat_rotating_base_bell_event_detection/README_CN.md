# ESP-VoCat-Base: 喵伴旋转底座（铃铛事件检测）

**中文** | [English](README.md)

## 项目简介

> **本例程专注于铃铛事件检测场景**：在综合例程（`esp_vocat_rotating_base`）的基础上，针对铃铛配件加载了铃铛专用磁场检测配置（`magnetic_slide_switch/profiles/bell`），并注册了磁开关事件回调，将检测到的事件通过 UART 主动上报给 ESP-VoCat 主机。
...
└── slide_switch_event_detect_task // 磁吸滑动开关事件检测任务（铃铛事件 → UART 上报）
```
